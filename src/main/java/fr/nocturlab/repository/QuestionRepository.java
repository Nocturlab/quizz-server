...
public interface QuestionRepository extends CrudRepository<Question, Integer> {
    ...
    @Query(value = "SELECT * FROM question q2 WHERE q.id IN (SELECT q.id, abs(q.difficulty - (SELECT difficulty FROM account WHERE id=1)) as difficulty FROM question q WHERE q.id not in (SELECT distinct r.question_id FROM resultat r WHERE r.account_id = 1) ORDER BY difficulty LIMIT 10)", nativeQuery = true)
